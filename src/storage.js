module.exports = function(app)
{
	...
	app.sql = new Database(app.config.server.database, { memory: false, readonly: false });
	...
	{
		// User management.
		addUser:							app.sql.prepare(Filesystem.readFileSync('sql/add_user.sql', 'utf8').trim()),
		getUser:							app.sql.prepare(Filesystem.readFileSync('sql/get_user.sql', 'utf8').trim()),
		getUserByAddress:					app.sql.prepare(Filesystem.readFileSync('sql/get_user_by_address.sql', 'utf8').trim()),
		listCampaignsByUser:				app.sql.prepare(Filesystem.readFileSync('sql/list_campaigns_by_user.sql', 'utf8').trim()),
		listContributionsByUser:				app.sql.prepare(Filesystem.readFileSync('sql/list_contributions_by_user.sql', 'utf8').trim()),
		listRecipientsByUser:				app.sql.prepare(Filesystem.readFileSync('sql/list_recipients_by_user.sql', 'utf8').trim()),

		// Commitment management.
		addCommitment:						app.sql.prepare(Filesystem.readFileSync('sql/add_commitment.sql', 'utf8').trim()),
		...
		addRecipientToCampaign:				app.sql.prepare(Filesystem.readFileSync('sql/add_recipient_to_campaign.sql', 'utf8').trim()),
		addContributionToCampaign:			app.sql.prepare(Filesystem.readFileSync('sql/add_contribution_to_campaign.sql', 'utf8').trim()),
		addContributionRevocation:			app.sql.prepare(Filesystem.readFileSync('sql/add_contribution_revocation.sql', 'utf8').trim()),
		addCampaignFullfillment:				app.sql.prepare(Filesystem.readFileSync('sql/add_campaign_fullfillment.sql', 'utf8').trim()),
		linkCommitmentToContribution:			app.sql.prepare(Filesystem.readFileSync('sql/link_commitment_to_contribution.sql', 'utf8').trim()),
	};
