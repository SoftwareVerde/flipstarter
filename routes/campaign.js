...
const campaignInformation = async function(req, res)
{
	...
	req.app.debug.server(`Campaign #${req.params['campaign_id']} data requested from ` + req.ip);
	...
	// Notify the server admin that a campaign has been requested.
	req.app.debug.server(`Campaign #${req.params['campaign_id']} data delivered to ` + req.ip);
